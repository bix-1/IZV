...
def plot_geo(gdf: geopandas.GeoDataFrame, fig_location: str = None,
    ...
    gdf = gdf[(gdf["region"] == "JHM") & (gdf["p36"] <= 1) &
              (gdf["date"].dt.year <= 2020)].to_crs("EPSG:3857")

    rows, cols = 3, 2
    fig, axs = plt.subplots(rows, cols, figsize=(15, 10))

    _, miny, maxx, maxy = gdf.total_bounds
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            gdf[(gdf["p36"] == c) & (gdf["date"].dt.year == (2018 + r))
                ].plot(ax=axs[r, c], markersize=2, color=colors[c])
            ctx.add_basemap(axs[r, c], crs=gdf.crs.to_string(), source=ctx.providers.Stamen.TonerLite,
                            alpha=0.9, zoom=10)
